...
//add an onclick to each square in your grid
 squares[i].onclick = function(){
    //if the the square below your current square is taken, you can go on top of it
...
function checkBoard(){
    ...
    if(square1.classList.contains('player-one') &&
       ...
       square4.classList.contains('player-one')) {
           //if they do, player-one is the winner
        result.innerHTML = 'Player one wins!'
       }
        //now check to if they have the class name of player-two
...
//add an eventListener to each square that will trigger the checkboard()
